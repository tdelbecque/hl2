...
object indexWithAbstract extends App {
  ...
  def createDocument (
    ...
    val abstractFieldType = new FieldType
    ...
    if (_abstract != null) {
      abstractFieldType setIndexOptions IndexOptions.DOCS_AND_FREQS_AND_POSITIONS_AND_OFFSETS
      abstractFieldType setStored false
      abstractFieldType setStoreTermVectors false
      d add new Field ("abstract", _abstract, abstractFieldType)
    }
  ...
  def indexDB {
    ...
    def addToIndex (pii: String, title: String, _abstract: String, objectTokens: ListBuffer [String], verbTokens: ListBuffer [String], subjectTokens: ListBuffer [String]) {
      val d = createDocument (pii, title, _abstract, objectTokens, verbTokens, subjectTokens)
      w addDocument d
    }
    ...
    try {
      ...
      val rs: ResultSet = stmt executeQuery ("select a.pii pii, title, abstract, hlno, predicates from articles a left outer join hung_predicates b on a.pii = b.pii order by pii, hlno")
      ...
      while (rs next) {
        hlno = rs.getInt ("hlno")
        if (hlno < 2 && pii != null) {
          addToIndex (pii, title, _abstract, objectTokens, verbTokens, subjectTokens)
          pii = null
        }
        if (hlno == 1) {
          objectTokens.clear
        ...
        _abstract = rs.getString ("abstract")
